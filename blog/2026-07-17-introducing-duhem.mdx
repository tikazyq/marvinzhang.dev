...
- **Drift resistance.** You write *what* to verify (intent), not *how* (the program tangled up with selectors and timing). An agent can rewrite the implementation and the check still means the same thing — no edit needed; a selector-coupled test breaks on every refactor.
