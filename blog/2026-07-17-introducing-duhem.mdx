...
| Signal | What it actually proves | What it can't see |
| ------ | ----------------------- | ----------------- |
| The AI's "all tests pass" | The agent believes the work is done | Whether tests really ran, and against the real system |
| Unit tests | A single function behaves correctly in isolation | How the parts behave once assembled |
| Integration tests with mocks | The stand-in works | Real configuration, data, and runtime |
| Container health checks | The process is alive | Whether the path users actually take still works |
...
Binaries are on [GitHub Releases](https://github.com/onsager-ai/duhem/releases); the [getting-started guide](https://github.com/onsager-ai/duhem/blob/main/docs/getting-started.md) walks you from zero to authoring your first check. Browser-driven (`ui/*`) checks additionally need a one-time `duhem browser install`.
