...
A fair question at this point: if κ can be pressed down, what about σ? It can — engineers have been pressing it forever: more staging environments, more CI concurrency; you can always buy another stove. But the two knobs differ in one crucial way: σ is the linear term and can usually be bought down with money, and a problem money can solve is not where scarcity lives; κ is quadratic, immune to spending, curable only by design. Apply the same 4× reduction to each knob and watch how differently the curve responds:

![σ can be bought, κ must be designed: the same 4× reduction lifts the curve when applied to σ (N* 6.8→7.0) but doubles the peak position when applied to κ (N* 6.8→13.6)](/img/blog/2026-07-13-multi-agent-teams-and-the-measurable-coordination-tax/figure-5-sigma-kappa.en.png)
...
![The skill migration: the λ era's hard currency is being wholesaled by agents, while the κ era's four crafts — decomposition, protocol design, verification standards, orchestration — become the new scarcity](/img/blog/2026-07-13-multi-agent-teams-and-the-measurable-coordination-tax/figure-6-skills.en.png)
