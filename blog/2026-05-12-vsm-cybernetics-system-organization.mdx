...
Layered, microservices, hexagonal, DDD — the four dominant architecture languages of the last two decades all answer the same question: *how is the code partitioned?* None has a word for the question the team asked on Tuesday: *how does this system, as an organisation, stay alive?* That question has always been answered, but somewhere else — in the human organisation around the code. PMs and scrum masters ran the schedule. SRE and QA ran the audit. Product watched the market. Executives set direction. Incident responders pulled the alarm. The architecture languages didn't need vocabulary for any of this because the org chart absorbed it.

Cybernetics named those parts fifty years ago, then software discourse forgot the vocabulary. Norbert Wiener (1948), Ross Ashby (1956), and Stafford Beer (1959–85) worked out what any system — biological, mechanical, organisational — needs to preserve its identity under disturbance: parts that coordinate, schedule, audit, look outward, hold purpose, fire alarms. The vocabulary disappeared from mainstream software discourse, reasonably, because the human layer around the code carried those functions out-of-band. AI agent systems are starting to change that. When agents take over the operational layer, the surface humans need to cover grows faster than the human layer can stretch — and the cybernetic vocabulary becomes useful again, because the parts it names now have to be *designed in*.

The **Viable System Model**, published by Beer in 1972 and derived from Ashby's earlier theorems, is the diagnostic mirror best suited to this moment. Not because it offers an architectural pattern to copy — it does not — but because it gives names to the parts the dominant architecture languages have no words for. The rest of this article walks the cybernetic lineage, applies VSM's five subsystems plus its algedonic alarm channel to a generic AI-agent pipeline, states three counterintuitive diagnostic heuristics, and shows why Ashby's Law of Requisite Variety and the Conant-Ashby theorem imply that governing AI agents with hard rules is structurally guaranteed to fail.
