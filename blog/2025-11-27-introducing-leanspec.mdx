...
title: "Introducing LeanSpec: A Lightweight SDD Framework Built from First Principles"
authors: ["marvin"]
...
New to SDD? Start with my foundational article [Spec-Driven Development: A Systematic Approach to Complex Features](/blog/spec-driven-development) for methodology basics, or dive into the [2025 SDD Tools Landscape](/blog/sdd-tools-practices) for a comprehensive comparison of industrial tools. Want to try the methodology without installing anything? The [Practice SDD Without the Toolkit](https://www.lean-spec.dev/docs/tutorials/sdd-without-toolkit) tutorial has you covered.
:::
...
```mermaid
flowchart LR
    subgraph Principles["First Principles"]
        CE["🧠 Context Economy"]
        SN["📊 Signal-to-Noise"]
        IO["🎯 Intent Over Implementation"]
        BG["🌉 Bridge the Gap"]
        PD["📈 Progressive Disclosure"]
    end
    
    CE --> Spec["Spec < 300 lines"]
    SN --> Spec
    IO --> Spec
    BG --> Spec
    PD --> Spec
    
    Spec --> Human["Human Understanding"]
    Spec --> AI["AI Understanding"]
...
If you've read my [SDD Tools analysis](/blog/sdd-tools-practices), you know I evaluated six major tools in this space. Here's where LeanSpec fits:
