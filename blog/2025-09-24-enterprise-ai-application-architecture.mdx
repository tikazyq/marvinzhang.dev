...
```python
# Function call that appears to work but returns inconsistent data
def get_order_status(order_id):
    # Returns cached data that's 30 minutes behind
    # Customer sees "shipped" in email, but this returns "processing"
    return order_system.query(order_id)  # Stale cache hit
```
...
In the journey of building enterprise LLM applications and data infrastructure, the following anti-patterns are especially common and worth avoiding:

1. **Dumping large raw documents directly into prompts**
    - Assuming "bigger context windows" means you can stuff all knowledge in, leading to wasted tokens, diluted information, and models missing the point.
2. **Ignoring data freshness and consistency**
    - Tool calls/retrievals focus only on "can I get a value" without checking if data is stale or conflicting across sources, resulting in business decisions based on outdated facts.
3. **RAG retrieval without reranking or deduplication**
    - Relying solely on embedding similarity for retrieval, without semantic reranking or deduplication, so the model receives redundant, conflicting, or even self-contradictory context.
4. **Unstructured error handling in tool/function calls**
    - Tool call failures only return error text, with no structured error codes/freshness/degradation signals, so the model can't recover or explain issues sensibly.
5. **Missing governance and lineage, making incident review impossible**
    - No traceable chain for context assembly/tool calls/decision references, so when things go wrong, root cause analysis is guesswork, not systematic improvement.

These anti-patterns usually stem from focusing only on model capabilities while neglecting the data, tooling, and governance foundations. Spotting and avoiding them is the first step toward scalable, reliable LLM applications.
...
**The strategic imperative is clear**: Organizations that build LLM applications on solid data foundations will consistently deliver more reliable, scalable, and valuable intelligent systems than those focused primarily on model capabilities or agent frameworks.
