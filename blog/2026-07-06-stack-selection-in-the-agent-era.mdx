...
**Step three: compare only within the training-data mainstream.** The axes assume first-draft quality is roughly saturated across candidates. Put an exquisite niche language on this chart and it will look better than it performs, because its real handicap — a weak first draft on every round — appears on neither axis. That is exactly what the asterisk on Dart marks: near the sweet spot on pure axes, thin in the corpus. This is the legitimate core of the training-data objection, absorbed here as a usage boundary.
