...
![Two panels: in the same hour an agent gets 6 rounds on a slow loop versus 60 on a fast one; and from the same first draft, dense verification signals converge defects toward zero round by round while sparse signals plateau and leak to humans and production — delivery speed and quality both compound with the loop](/img/blog/2026-07-06-web-stack-selection-in-the-agent-era/figure-3-compounding.en.png)
...
If the two criteria are real, they should already be visible in the world: someone should be paying for them, and someone should be voting with them. The three exhibits below are that trail of money and votes — and each lands, with almost suspicious precision, on an argument from the 2021 series.
...
![A two-axis plane — loop latency versus verification signal density. TS 7 native, Go, C# and Hono + Zod sit in or near the joint-DX sweet spot; Rust, Kotlin, Java and TS on the JS-based tsc are trustworthy but slower; PHP, Ruby, plain JavaScript and typical Python setups are fast but sparse on signals; C/C++ lands in agent hard mode; Bun today is furthest right with a reliability discount.](/img/blog/2026-07-06-web-stack-selection-in-the-agent-era/figure-6-quadrant.en.png)
...
**Step three: compare only within the training-data mainstream.** The axes assume first-draft quality is roughly saturated across candidates. Put an exquisite niche language on this chart and it will look better than it performs, because its real handicap — a weak first draft on every round — appears on neither axis. This is the legitimate core of the training-data objection, absorbed here as a usage boundary.
