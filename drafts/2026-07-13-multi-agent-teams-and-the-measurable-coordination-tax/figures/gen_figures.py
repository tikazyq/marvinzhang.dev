...
emit("figure-1-archaeology", "en", "Fig. 1 — Three old posts, one question", "FIG 01 — SITE ARCHAEOLOGY",
     ...
     "marvinzhang.dev", "FIG 1 / 6", F1_CSS)
...
emit("figure-3-evidence", "zh", "图 3 — agent 团队的读数", "图 03 — 实证读数",
     ...
     "arXiv 2512.08296 · 2503.13657 · 2603.01045", "图 3 / 6", F3_CSS)
...
emit("figure-3-evidence", "en", "Fig. 3 — Readings from agent teams", "FIG 03 — THE RELAPSE",
     ...
     "arXiv 2512.08296 · 2503.13657 · 2603.01045", "FIG 3 / 6", F3_CSS)
...
emit("figure-4-walls", "en", "Fig. 4 — Three walls come down", "FIG 04 — MEASURABILITY",
     ...
     "marvinzhang.dev", "FIG 4 / 6", F4_CSS)
...
# ---------------------------------------------------------------- figure 5 (new): sigma vs kappa
F5B_CSS = """
  .duo { display: flex; gap: 16px; }
  .pan { flex: 1; border: 1.5px solid; border-radius: 12px; background: #fff; padding: 15px 16px 12px; }
  .p-sig { border-color: var(--info-stroke); } .p-kap { border-color: var(--hi-stroke); }
  .pan h2 { margin: 0 0 1px; font-size: 16px; font-weight: 600; }
  .p-sig h2 { color: var(--info-stroke); } .p-kap h2 { color: var(--hi-stroke); }
  .pan .sub { font-size: 12.5px; color: var(--ink-soft); margin-bottom: 8px; }
  .pan svg { display: block; }
  .verdict { margin-top: 9px; border-radius: 9px; padding: 8px 12px; font-size: 13px; line-height: 1.45; }
  .p-sig .verdict { background: var(--info-fill); color: var(--info-stroke); }
  .p-kap .verdict { background: var(--hi-fill); color: var(--hi-stroke); }
  .verdict b { font-family: 'JetBrains Mono', monospace; }
"""

def usl_x(N, lam, sig, kap):
    return lam * N / (1 + sig * (N - 1) + kap * N * (N - 1))

def usl_panel(sig2, kap2, color, w=340, h=190, ymax=5.2):
    import math
    padL, padR, padT, padB = 30, 10, 26, 22
    pw, ph = w - padL - padR, h - padT - padB
    def mx(N): return padL + (N - 2) / 22.0 * pw
    def my(v): return padT + (1 - v / ymax) * ph
    def path(sig, kap):
        pts = []
        N = 2.0
        while N <= 24.001:
            pts.append(f"{mx(N):.1f},{my(usl_x(N,1,sig,kap)):.1f}")
            N += 0.5
        return "M" + "L".join(pts)
    base_s, base_k = 0.08, 0.02
    ns_base = math.sqrt((1 - base_s) / base_k)
    ns_new = math.sqrt((1 - sig2) / kap2)
    xb, yb = mx(ns_base), my(usl_x(ns_base, 1, base_s, base_k))
    xn, yn = mx(ns_new), my(usl_x(ns_new, 1, sig2, kap2))
    return f"""<svg width="{w}" height="{h}" viewBox="0 0 {w} {h}">
<line x1="{padL}" y1="{padT}" x2="{padL}" y2="{h-padB}" stroke="#8a8a8a" stroke-width="1"/>
<line x1="{padL}" y1="{h-padB}" x2="{w-padR}" y2="{h-padB}" stroke="#8a8a8a" stroke-width="1"/>
<text x="{mx(2)}" y="{h-7}" font-size="10" fill="#8a8a8a" text-anchor="middle">2</text>
<text x="{mx(13)}" y="{h-7}" font-size="10" fill="#8a8a8a" text-anchor="middle">N</text>
<text x="{mx(24)}" y="{h-7}" font-size="10" fill="#8a8a8a" text-anchor="middle">24</text>
<path d="{path(base_s, base_k)}" fill="none" stroke="#b0b0b0" stroke-width="1.8" stroke-dasharray="5 4"/>
<path d="{path(sig2, kap2)}" fill="none" stroke="{color}" stroke-width="2.6"/>
<line x1="{xb:.1f}" y1="{yb:.1f}" x2="{xb:.1f}" y2="{h-padB}" stroke="#b0b0b0" stroke-width="1" stroke-dasharray="3 3"/>
<line x1="{xn:.1f}" y1="{yn:.1f}" x2="{xn:.1f}" y2="{h-padB}" stroke="{color}" stroke-width="1.2" stroke-dasharray="3 3"/>
<circle cx="{xb:.1f}" cy="{yb:.1f}" r="3.5" fill="#b0b0b0"/>
<circle cx="{xn:.1f}" cy="{yn:.1f}" r="4" fill="{color}"/>
<text x="{xn:.1f}" y="{padT-9}" font-size="11.5" font-weight="700" fill="{color}" text-anchor="middle" font-family="JetBrains Mono, monospace">N* {ns_base:.1f} → {ns_new:.1f}</text>
</svg>"""
