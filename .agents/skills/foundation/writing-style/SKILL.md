...
own hand-written articles (2021–2023). Three hard rules, all learned from
...
3. **No AI sentence-rhythm drift** (measured, 2026-08). The tells are not bad
   words, they are overuse. Across ~64,000 characters of the author's
   hand-written work there is **not one em dash（——）**; AI-assisted posts went
   3 → 41 → 94 per article over eighteen months, one draft at a time. 「恰恰」
   shows the same curve. 「不是……而是」is *not* banned — the author uses it about
   once per article to clear up a specific misreading — but a model reaches for
   it as rhythm every few paragraphs, and that shows.
   **Run `pnpm run validate:zh-voice`**; it checks each file against a committed
   baseline (never worse) and holds new files to the author's own measured rate.
...
4. **中文标题不预设读者知识**: a title must land for someone who has never
...
5. **符号随行** (symbols ride along): for general-reader articles, Greek
