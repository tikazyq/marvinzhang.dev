...
| The article's editorial header comment appears in the export | `wechat.js` stripped only `{/* ... */}`, so posts whose ZH source uses `<!-- ... -->` shipped their whole drafting header (invariants, voice red lines, notes on author decisions) into the paste source. Fixed 2026-08: both comment syntaxes are stripped. Caught by review on PR #57, not by reading the export — an HTML comment is invisible in a rendered preview, so check `grep -c '<!--'` on the artifact rather than trusting your eyes |
